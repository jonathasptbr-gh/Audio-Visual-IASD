...
| `casca-contrato.test.mjs` | **as listas da segunda casca que moram em DUAS LINGUAGENS** — irmão do `tipos-que-sobem`, e pelo mesmo motivo: uma lista escrita duas vezes diverge no primeiro esquecimento, e a divergência é MUDA. São três: o degrau do contrato (`NativeBridge.SHELL_VERSION` × `Programa.SHELL` — a superfície da ponte é UMA, e o número tem de ser um), o que atravessa o cano (`DA_CASCA` × os `case` de `Folhas.cs`) e a lista da invariante 9. **Ele cobra também que a tabela do `docs/SEGUNDA-CASCA.md` §7 não envelheça em relação ao código** — é a defesa contra o pior artefato que este projeto sabe produzir, um documento que continua respondendo, com a frase errada |
