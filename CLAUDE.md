...
| peça | onde |
|---|---|
| a seção da folha desliza ao abrir e ao fechar | `pacoteAnimarSecao` + `expandAccordion`/`collapseAccordion` |
| a gaveta de opções de play rola para caber | `revelarNaLista` + `scrollerDe` |

> **O ACORDEÃO É O MESMO, INCLUSIVE NA ASSIMETRIA** (v1.8.42). Pedido do
> operador: *"na biblioteca temos animações de abertura e fechamento das listas
> e grupos, faça essa animação ali no exportar também"*. São as MESMAS funções e
> o mesmo desenho da `alternar` da Biblioteca, e a assimetria é o ponto:
> **FECHANDO anima ANTES de remontar** (o redesenho apaga o nó, e um nó apagado
> não desliza), **ABRINDO remonta primeiro** e a animação espera o quadro
> seguinte (o corpo novo ainda não está no documento quando `remontar()`
> devolve, e fora da árvore toda medida é zero). `pacoteAnimarSecao` é o irmão
> do `gruposAnimar` de lá, e existe pela mesma razão: sem a marca, todo
> redesenho reanimaria a seção aberta — e marcar um álbum lá dentro remonta a
> folha inteira.

> **REVELAR NÃO É ALINHAR AO TOPO, E A DISTINÇÃO É O RECURSO** (v1.8.42).
> Relato: *"na tela que temos para as coleções e álbuns [há rolagem], mas que
> não temos para as opções de play … constantemente a visualização das opções de
> play depende de rolar a tela manualmente para ver elas"*.
>
> O `alinharGrupoNoTopo` responde a OUTRA pergunta, e é por isso que ele não
> serve aqui: uma coleção que abre VIRA o assunto da tela, e alinhá-la ao topo é
> o pedido da v5.277 ao pé da letra. Uma FAIXA continua sendo uma de muitas — o
> que falta não é pô-la no topo, é a gaveta dela CABER. Alinhar uma faixa do
> meio de um hinário ao topo jogaria fora o contexto que o operador estava
> percorrendo, e para uma faixa que já está no alto não haveria movimento nenhum
> a fazer.
>
> **A regra é uma só e cobre os dois casos:** o par (linha + gaveta) transborda
> embaixo → rola o MÍNIMO para caber; o par é mais alto que a caixa → alinha o
> TOPO da linha, que é o teto do que a rolagem pode fazer. **O scroller é
> PROCURADO e não recebido** (`scrollerDe`): a mesma linha é montada em quatro
> telas e cada uma rola numa caixa diferente — um parâmetro obrigaria os quatro
> chamadores a saber o que a árvore já responde, e o quinto nasceria sem ele.
>
> **DEPOIS DA ANIMAÇÃO** (`ACC_MS + 30`), pelo motivo do `alinharGrupoNoTopo`:
> durante os 220 ms o corpo ainda cresce de zero, e medir ali dá a geometria da
> linha FECHADA. MEDIDO no oráculo: sem a chamada sobram **196px** de gaveta
> fora da caixa; com ela, zero.

**O QUE O LOTE TRAZ (v1.8.41) — a folha de exportação medida CONTRA a
Biblioteca, em vez de parecida com ela:**

| peça | onde |
|---|---|
| a seta do acordeão é a `.coll-group-icon` de lá, pela classe | `chevronUpIconSvg` + `.pacote-seta` |
| o quadrado do ícone de uma linha é a `.coll-bar-icon` de lá | o `ic.classList.add` do `linhaDeGrupo` |
| o preenchimento de ESCOLHIDO saiu desta folha; a marca é o ✓ | a regra de `--pacote-superficie` (`controle.css`) |
| o recuo de quem está dentro voltou ao da Biblioteca | o `.song-menu-dentro`, removido |

> **A SEGUNDA RODADA DO MESMO PEDIDO, E ELA MUDOU A RÉGUA** (v1.8.41). Relato do
> operador, com o degrau da v1.8.40 já no aparelho: *"a seta de abertura do
> acordeão está diferente, as cores entre o dono e os itens de sua lista também
> estão diferentes do que temos na biblioteca … eu quero o mesmo design da
> biblioteca, cores e ícones — o padrão da biblioteca já temos, não precisamos
> de algo diferente"*.
>
> **AS DUAS METADES ESTAVAM CERTAS, e a v1.8.40 não tinha como ver nenhuma:**
> ela mediu um DEGRAU contra um piso (`> 1,25`), e um degrau prova que a folha
> tem hierarquia — não que ela tem A hierarquia da Biblioteca. **A régua passou
> a ser a própria Biblioteca**, montada na MESMA página pelo caminho de verdade
> (`renderCollectionsList`) e comparada eixo a eixo. Um número escrito no
> oráculo envelheceria à parte do app na primeira vez que a Biblioteca mudasse
> de tom, e a divergência voltaria sem nada reprovar.
>
> **A SETA não tinha um eixo em comum** (MEDIDO): lá um quadrado de `--hit` em
> `--btn-accent`, raio 8px, chevron de traço 2,4; aqui um chevron de traço 2
> solto numa caixa de 24px transparente. Hoje é a **mesma classe** — a
> `.coll-group-icon` — e a mesma função de desenho (`chevronUpIconSvg`), porque
> uma segunda descrição do mesmo objeto divergiria no primeiro ajuste de lá. O
> que sobra de próprio é qual estado gira: o seletor da Biblioteca fala da
> árvore dela, e aqui a marca de aberto é uma classe da própria seta.
>
> **AS CORES: o tom existia e nunca era visto.** MEDIDO no estado em que a folha
> de fato abre — barra e linhas em `rgb(46, 66, 98)`, **a mesma cor**, contra
> `rgb(52, 71, 91)` e `rgb(33, 47, 61)` na Biblioteca. A causa é o `--sel-fill`
> da linha marcada, e ele está CERTO onde nasceu: na folha de DESTINOS nada
> começa marcado, e ali o preenchimento É a resposta. **Aqui tudo nasce marcado**
> — o trabalho desta folha é TIRAR —, então ele pinta 100% das linhas, não
> distingue nenhuma, e o que apaga é a única coisa que responde *"o que é topo e
> o que é item?"*. É a regra da v5.151 num lugar novo: *uma condição quase
> sempre verdadeira não ADIA a pergunta, ela a APAGA*. Quem carrega a marca é o
> ✓, que já está em toda linha; o que saiu foi a segunda cópia da resposta.
>
> **E O RECUO DE TEXTO SAIU JUNTO**, pelo mesmo argumento: ele existia (v1.7.3)
> porque não havia bloco, e era a única coisa que dizia a quem uma linha
> pertencia. Com o corpo DENTRO do bloco (v1.8.40) isso passou a ser dito por
> CONTENÇÃO, e o recuo virou a segunda cópia — MEDIDO, 39,2px contra 6,4px na
> Biblioteca. Hoje: **5,59px**.
...
> **O `ResizeObserver` NÃO REPÕE NADA AQUI, e isto foi MEDIDO** (v1.8.37). O
...
> **UM ÍCONE DE BARRA OU ANDA OU DIZ UM NÚMERO, NUNCA OS DOIS** (v1.8.34).
...
| peça | onde |
|---|---|
| a folha recorta o que é dela | `.tools-sheet { overflow: hidden }` |
| o oráculo que mede DURANTE o movimento, em PIXEL | `tools/deslize-nao-vaza.test.mjs` |

> **A FOLHA É UM CARTÃO E NÃO RECORTAVA NADA** (v1.8.4). Relato do operador:
> *"os itens dentro das seções da bíblia estão aparecendo horizontalmente em
> sua animação de entrada e saída, saindo da caixa a qual pertecem"*.
>
> A navegação dentro da Bíblia desliza (`deslizarNaFolha`): um
> `translateX(±100%)` no `#bibleBody`, 220ms. MEDIDO a 393×786: **47,7px** de
> vazamento, 25 nós fora da caixa numa das transições. Quem já recortava era o
> `<main>`, e só na LARGURA DA TELA — entre a borda da folha e a da tela sobra
> a moldura em que o vazamento aparecia. **Pré-existente:** a mesma sonda
> contra o `controle.css` da v1.8.0 devolve os mesmos números.
>
> **E O ORÁCULO NÃO CABIA NO DE GEOMETRIA, por duas razões de método.** Aquele
> ASSENTA as animações antes de medir, e tem de assentar — logo um defeito que
> só existe DURANTE o movimento nasce fora do alcance dele. E **geometria não
> responde a esta pergunta**: `overflow: hidden` recorta a PINTURA, não o
> layout, então com a correção o `getBoundingClientRect` de cada ladrilho
> continua 47,6px fora da folha, e antes e depois medem IGUAL. A régua virou o
...
| o PADRÃO escrito: seis regras de geometria | `docs/arquitetura/DESIGN-SYSTEM.md` |

> **A CAIXA É DA TELA E ENCOLHE; O CONTEÚDO É DA FONTE E CRESCE** (v1.8.1).
...
> não são `rem`.
>
> As seis regras (G1 a G6) e o corolário — *numa folha, o CROMO é o que cede,
...
>
> **E um achado de passagem:** a v1.8.0 escreveu
...
**O QUE AQUELE LOTE TROUXE — dois relatos do operador, na mesma captura:**
...
> estivesse corrompido."* **São os dois, um em cada camada:**
>
...
>   erro nenhum, e o cursor tropeça no meio de um registro.
>
> **A FORMA DO CONSERTO É A DO `StreamProxy`:** a faixa vai na QUERY e nunca num
> cabeçalho `Range` — com o cabeçalho, o `ParseRange` do WebView aplicaria o
> deslocamento uma SEGUNDA vez sobre o que já é uma fatia. Sem cabeçalho não há
> `ParseRange`, não há `ComputeBounds`, e o `available()` passa a ser o da
> JANELA. **O teto de 2 GB deixa de existir porque nenhuma resposta chega perto
> dele.**
>
> **E A LEITURA ANTECIPADA CRESCE E ENCOLHE**, porque uma janela fixa erra nos
> dois regimes do formato — e foi o oráculo que pegou isso: com 4 MB fixos, a
> conferência de um acervo lia 4 MB para aproveitar duzentos bytes, uma vez por
> registro. A regra é o próprio percurso: pedido que começa onde o buffer acabou
