...
> **UM CONTROLE RESPONDE POR RECUO; UM BLOCO RESPONDE POR LUZ** (v1.7.2), e o
...
> **O `⋮` CEDE A COLUNA AO TRABALHO EM CURSO** (v1.7.2) — o mesmo movimento que
...
> **A LINHA EXCLUÍDA NÃO VOLTA AO ESTADO INICIAL PARA SUMIR** (v1.7.2). Relato:
...
> **A `object-URL` DE UMA MINIATURA É DO BLOB, NÃO DO RENDER** (v1.7.2). A
...
> **O LOTE ANTERIOR (v1.7.1)** também foi só de web, e trouxe a legenda do
> trabalho para a posição do subtítulo — *"gostaria que usasse a posição do texto
> secundário … e a fração das páginas já preparadas"* — mais a SETA da miniatura
> seguindo a LEGENDA (`legendaEhDownload`, a fonte única do cartão e da linha:
> preparar não baixa byte nenhum). A barra que ele desenhou ao lado da frase saiu
> na v1.7.2, a pedido; a legenda, que é o que acrescentou informação, ficou.

> **E O ANTERIOR A ELE (v1.7.0)** pediu Release, porque a ponte ganhou QUATRO métodos:
