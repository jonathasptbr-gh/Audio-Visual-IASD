...
| peça | onde |
|---|---|
| a margem do retrato zerada na coluna | `:fullscreen .lv-cifra-ctl { margin-right: 0 }` |
| o eixo X, que a coluna nunca media | `cifra-tela-cheia.test.mjs` (o par de centros) |

> **LOTE SÓ WEB.** Só `controle.css` e um oráculo — daí o `version.json` sair
> SEM `shellTag`. O APK segue na v1.8.29, que é o que o Registro vai mostrar.
>
> **A ARMADILHA, para o próximo bloco que trocar de eixo:** uma margem `auto`
> escrita para a LINHA do retrato não é neutra na COLUNA da tela cheia — no
> eixo transversal ela VENCE o `align-items` do pai e encosta a caixa num
> lado. O CSS já desfazia isso para o `.lv-cheia-btn` (*o respiro da saída
> troca de eixo com a fila*) e não para a fila. **Regra: todo bloco que vira
> coluna em `:fullscreen` confere as margens que o retrato deixou.**
