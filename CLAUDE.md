...
> **O DIA E AS LINHAS ERAM O MESMO PIXEL** (v1.7.4). Relato do operador:
...
> **O LOTE ANTERIOR (v1.7.3) — a resposta de uma ação nasce ONDE ela foi
...
> com a de um download:** a FAIXA de progresso na posição do subtítulo com a
> FRAÇÃO das páginas (`faixaDeProgresso`/`pintarFaixaDeProgresso`), e a SETA
> seguindo a LEGENDA também na linha, porque preparar não baixa byte nenhum
> (`legendaEhDownload`, a fonte única dela e do cartão da preview).
