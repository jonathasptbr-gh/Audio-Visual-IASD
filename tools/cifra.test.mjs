...
// ── 14. O BUSCADOR EXTERNO ──────────────────────────────────────────────────
// A busca do próprio Cifra Club NUNCA teve como funcionar: MEDIDO num aparelho,
// `?q=<termo>` responde 425 kB cujos únicos links de duas partes são o índice
// A–Z e o "Academy" — os resultados são desenhados por JavaScript, que o
// `cifraHtml` não executa. O caminho que resta é perguntar a um buscador com o
// site travado na consulta.
//
// Trocar de motor não pode trocar o CRITÉRIO: o que sai daqui tem a mesma forma
// do `lerBusca`, e quem julga continua sendo o `ordenarBusca`.
secao('14. buscador externo');
{
  checar(C.urlDeBuscaExterna('Unidos em Cristo').startsWith('https://html.duckduckgo.com/html/?q='),
    'a consulta vai ao endpoint HTML (sem JS, servido pelo servidor)', C.urlDeBuscaExterna('Unidos em Cristo'));
  checar(C.urlDeBuscaExterna('Unidos em Cristo').includes('site%3Acifraclub.com.br'),
    'com o SITE travado — sem isso os primeiros resultados são de outros cinco sites');
  checar(C.urlDeBuscaExterna('  ') === '', 'consulta vazia não produz URL');

  // O DESEMBRULHO. O buscador entrega o resultado como um redirecionamento
  // (`/l/?uddg=<URL codificada>`), e um parser que só procurasse
  // `href="https://…"` não acharia resultado NENHUM — acharia a navegação, que
  // é o defeito deste lote repetido noutro site.
  checar(C.desembrulhar('//duckduckgo.com/l/?uddg=https%3A%2F%2Fwww.cifraclub.com.br%2Fa%2Fb%2F&rut=x')
    === 'https://www.cifraclub.com.br/a/b/', 'o redirecionamento é desembrulhado',
    C.desembrulhar('//duckduckgo.com/l/?uddg=https%3A%2F%2Fwww.cifraclub.com.br%2Fa%2Fb%2F&rut=x'));
  checar(C.desembrulhar('https://www.cifraclub.com.br/a/b/') === 'https://www.cifraclub.com.br/a/b/',
    'e a forma DIRETA passa igual — a que vem depende da região e do dia');
  checar(C.desembrulhar('/interno/') === '', 'link relativo do buscador não é resultado');
  checar(C.desembrulhar('') === '', 'href vazio não explode');

  // A PÁGINA, com as duas formas e com o que NÃO pode entrar.
  const serp = [
    '<a href="/settings">Configurações</a>',
    '<a href="//duckduckgo.com/l/?uddg=https%3A%2F%2Fwww.cifraclub.com.br%2Fartista-marcador%2Fmusica-marcador%2F&rut=1">',
    'Musica Marcador - Artista Marcador - Cifra Club</a>',
    '<a href="https://www.cifraclub.com.br/outro-marcador/outra-marcador/">Outra Marcador</a>',
    // OUTRO SITE não entra: o app lê cifra de UM lugar, e um resultado
    // patrocinado não pode virar a folha do culto.
    '<a href="https://www.letras.mus.br/artista/musica/">Musica Marcador</a>',
    // E O DOMÍNIO PARECIDO tampouco — a conferência é por HOST, nunca por
    // prefixo: `cifraclub.com.br.exemplo.com` qualquer um registra.
    '<a href="https://www.cifraclub.com.br.exemplo.com/a/b/">Musica Marcador</a>',
  ].join('');
  const r = C.lerBuscaExterna(serp);
  checar(r.length === 2, 'só os dois resultados do Cifra Club entram', r.map((x) => x.url));
  checar(r[0].url === 'https://www.cifraclub.com.br/artista-marcador/musica-marcador/',
    'o embrulhado vira endereço absoluto do site', r[0] && r[0].url);
  checar(!r.some((x) => /exemplo\.com/.test(x.url)),
    'o domínio PARECIDO fica de fora — a conferência é por host', r.map((x) => x.url));
  checar(!r.some((x) => /letras\.mus\.br/.test(x.url)), 'e outro site também');

  // O SUFIXO DO TÍTULO sai: ele é constante em todos os resultados, então não
  // separa nenhum — só encurta o que o parentesco tem para comparar.
  checar(r[0].nome === 'Musica Marcador - Artista Marcador',
    'o "- Cifra Club" do fim do título sai', r[0] && r[0].nome);

  // A FORMA É A DO `lerBusca`, e é isso que faz o juiz continuar o mesmo.
  const ord = C.ordenarBusca(r, 'Musica Marcador', '');
  checar(ord.length >= 1 && /musica-marcador/.test(ord[0].url),
    'e o `ordenarBusca` julga o resultado do buscador sem saber de onde ele veio', ord[0] && ord[0].url);

  checar(C.lerBuscaExterna('').length === 0, 'página vazia devolve lista vazia');
  // A NAVEGAÇÃO do buscador não pode virar resultado — o mesmo erro, no site novo.
  checar(C.lerBuscaExterna('<a href="/settings">Configurações</a><a href="/about">Sobre</a>').length === 0,
    'e uma página só de navegação também');
}
