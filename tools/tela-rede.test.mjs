...
// ---------------------------------------------------------------------------
// 5b-quater. (A TELA DA REDE NUNCA ABRE O MICROFONE — bloco removido na v1.8.89)
//
// Ele forjava uma `navigator.mediaDevices`, mandava `{ type: 'mic', on: true }`
// pelo SSE e contava as chamadas a `getUserMedia`. O que ele media era a guarda
// `if (TELA) return` no topo do `setMic` — a linha que impedia cada tela da
// rede de abrir o microfone DO APARELHO ONDE O NAVEGADOR RODA, devolvendo-o às
// caixas daquele mesmo aparelho.
//
// COM O `setMic` FORA, A ASSERÇÃO VIROU TAUTOLOGIA: nada trata o comando `mic`,
// então `getUserMedia=0` passa por vacuidade — e uma asserção que não pode
// reprovar o defeito que ela nomeia é pior que asserção nenhuma. Ela saiu por
// isso, não por escolha de cobertura.
//
// A PERGUNTA QUE SOBREVIVE É OUTRA, e ela mudou de arquivo: *"voltou algum
// caminho de captura à base web?"* — `tools/sem-captura.test.mjs`, que também
// guarda, por escrito, as três peças que um caminho novo teria de trazer de
// volta (a escada de três degraus, a guarda de papel e o dreno do
// `mic-status`). O AMBIENTE nunca foi a proteção: uma tela roda em `http://` e
// `navigator.mediaDevices` é `[SecureContext]`, mas isso se desfaz sozinho no
// dia em que a transmissão subir em `https://`.
// ---------------------------------------------------------------------------
