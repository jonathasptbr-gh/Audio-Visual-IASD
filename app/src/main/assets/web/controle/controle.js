...
const ICON = {
  ...
  // voltou na v5.257 como o botão de TRAZER uma pasta do aparelho — a única
...
async function syncGroup(key, label, colls, opts) {
  ...
  // coleções — saiu na v5.257 junto com o botão que a disparava, e ela era o
...
// DE ONDE O ITEM VEIO (v5.257, pedido do operador: *"não apenas o tipo de
...
function renderLibrary() {
  // O menu de uma linha (v5.257) não sobrevive ao redesenho — ver
  ...
  items.forEach((item) => {
    ...
    // TUDO ATRÁS DO `⋮` (v5.257) — ver `montarAcoesDaLinha`. A ordem dentro da
...
// popup, que saiu na v5.257; `aposClique` era a diferença legítima daquele
...
function renderCollectionsListMiolo(alvo, redesenhar, opts) {
  ...
  const grupo = (text, colls, gOpts) => {
    ...
    // A MARCA que o redesenho no lugar procura (v5.257, ver
  ...
  // (A barra "Toda a biblioteca" saiu na v5.257 — ver o bloco acima. Ela já
  ...
  const favCorpo = grupo(GRUPO_FAVORITOS, null, {
    ...
    //
    // PASTA + (v5.257, pedido do operador). A v5.254 pôs aqui as setas
...
// ===== "BAIXAR TODA A BIBLIOTECA" SAIU (v5.257) =====
//
...
function favItemRow(item) {
  ...
  // E OS TRÊS FICAM ATRÁS DO `⋮` (v5.257), como no Cronograma: era esta lista
...
//
// E DESDE A v5.257 ELE TAMBÉM REDESENHA (relato do operador: *"se estou na
