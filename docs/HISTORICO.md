...
## v1.4.4 — "há tela" nunca foi "há telão"; e o travamento que não aparecia

Dois relatos do mesmo sábado, e dois caminhos diferentes.

### 1. O som que só voltava relançando o app

> *"o volume não vinha independente de aumentar o volume ou não… e ficou
> travando… fechei tudo e abri do zero e daí veio som"*

**A frase do meio é o diagnóstico.** Só o relançamento recuperava — e o código
previa isso:

| pergunta | quem responde hoje | quem respondia |
|---|---|---|
| há uma TELA externa? | `DisplayManager` | `DisplayManager` |
| há PROJEÇÃO? | a `Presentation` | **o `DisplayManager`** |

As duas divergem exatamente durante uma negociação de Miracast: `p.show()`
**lança** com o dongle instável (o `syncPresentation` já tratava a exceção) e o
sistema derruba a janela sozinho numa oscilação (`setOnDismissListener`). Nos
dois casos **a tela continua listada**. Enquanto o web perguntou
`lastDisplays.length > 0`, esse estado calava a preview — havia "para onde mandar
o som" — sem ninguém tocando do outro lado: **silêncio nos dois lados**, sem erro
no console, e com o Registro afirmando "conectado", que é a frase que manda
investigar o app quando o que falhou foi a janela.

E **não passava sozinho**: `syncPresentation` só volta a rodar por um evento do
`DisplayManager` — que numa tela que continua listada não vem — ou por um
`onResume`. Num culto o celular fica no suporte. Fechar e reabrir o app era, por
construção, a única saída.

**Três metades, e nenhuma basta:**

1. **O campo `telao`** por tela (`listDisplays`), que passa a responder às três
   perguntas cuja resposta honesta é a JANELA e não a tela: quem toca o som
   (`somLocalDeveEstar`), se o microfone é oferecido (`haOndeReproduzirMic` —
   quem capta é o `/display/` DENTRO dela) e se o Modo Fácil destrava
   (`simpleDisplay`). O que segue lendo a lista CRUA é o que descreve a CONEXÃO:
   o rótulo da folha, o `applyPreviewAspect`, o Registro.
   **A tela CONTINUA na lista**, e é isso que separa o campo de um filtro:
   *"não há TV"* e *"a TV está aí e o telão não subiu"* pedem frases diferentes,
   e um filtro diria a primeira — que é falsa, com o cabo na mão.
2. **A escada de retomada** (`agendarRetomadaDoTelao`: 0,4 s → 8 s, cinco
   degraus). Cresce pelo motivo da retomada de áudio do `display.js` — o pior
   caso audível é uma falha no começo, não uma tentativa por quadro. Zera em toda
   subida, é **cancelada quando a tela some de verdade** (ali o caminho normal já
   é o certo) e **não é reagendada com um pedido em voo**: `onDisplayChanged`
   chega em rajada durante a negociação, e sem essa guarda a espera nunca
   cresceria.
3. **Com o telão no chão o som volta para o celular** — que no espelhamento
   continua chegando às caixas da igreja, porque o `REMOTE_SUBMIX` leva a mistura
   do aparelho inteiro (v1.1.7). O estado passou a ser DIZÍVEL em três lugares: a
   linha do tempo ("TV conectada, mas o TELÃO NÃO SUBIU"), o cabeçalho do
   Registro e o rótulo da folha de conexão.

### 2. O travamento que não aparecia

> *"veio som, porém ficou travando e qualidade de vídeo baixa"* — num vídeo do
> YouTube tocado direto do online.

O giro de espera (`.av-stage-busy`) existia **só na CARGA**: do comando ao
primeiro quadro. Uma parada por falta de buffer no meio do louvor congelava o
quadro sem nada na tela — e **um app quebrado produz exatamente a mesma
imagem**, o que deixava como única leitura possível um palpite ("deve ser a
estabilidade da internet"). A transmissão direta é a ÚNICA mídia do app que
precisa de JS rodando enquanto toca, e era a que menos sinal tinha.

- **Duas razões para o giro** (`esperaCarga` · `esperaBuffer`), que não se apagam
  uma à outra: com um dono só, o fim da carga apagaria um giro aceso por fome, e
  um `clear` deixaria o aro girando sobre o wallpaper.
- **A fome vira NÚMERO** (`AVStream.fome`, lido pelo Registro): episódios **e**
  segundos parados. Dois travamentos de meio segundo é uma rede que oscila; dez
  de cinco segundos é uma rede que não sustenta a faixa escolhida — e só a
  segunda tem resposta. Uma contagem sozinha não as distingue.
- **MEDIDO escrevendo o oráculo, e mudou o desenho:** um `MediaSource` nasce
  vazio e dispara `waiting` em **toda** transmissão. A primeira versão do censo
  contava a carga, e o número passava a dizer *"≥1 sempre"* — o mesmo que não
  dizer nada. A vigília só abre no primeiro `playing`.
- **Atraso de 600 ms e só no stream:** um arquivo local não fica sem dados, e um
  aro piscando a cada seek na frente da congregação é pior que aro nenhum.

### 3. A resolução baixa: a hipótese da internet está DESCARTADA

A leitura na hora foi *"a resolução estava baixa por questão da internet, ou por
garantia do vídeo não travar"* — a explicação certa para o **app do YouTube**, e
**falsa para este app**. `shared/mse.js` **não faz ABR**, e está escrito no
cabeçalho dele: o manifesto traz UMA faixa de vídeo, escolhida antes do primeiro
byte, e ela é servida até o fim. Aqui rede fraca produz **travamento** — que é
justamente o outro relato do mesmo sábado —, nunca imagem menor.

E o app também não escolhe baixo de propósito: o teto é 1080p, o padrão do
seletor é 1080p rearmado a cada item, e **MEDIDO em aparelho na v5.127** o caso
normal é `transmitindo 1080p (137@VISIONOS + 140@VISIONOS)` — o itag 137 É 1080p
AVC.

Sobram três causas, e o Registro passou a separar as duas primeiras
(`teto Xp → transmitindo Yp`, mais a nota `HAVIA Zp transmissível (outro
cliente)`):

| causa | como aparece |
|---|---|
| o VÍDEO não tem faixa alta | `teto 1080p → transmitindo 360p`, sem a nota |
| o OPERADOR pediu menos | `teto 480p → transmitindo 480p` |
| NÓS deixamos resolução na mesa | `transmitindo 360p · HAVIA 1080p transmissível` |
| o encoder do MIRACAST baixou a taxa | o Registro diz 1080p e a TV continua ruim |

**A terceira é a única que seria defeito nosso**, e ela é possível porque
`candidatosVideo` ordena por `ordemCliente` ANTES da altura. **Não foi
corrigida de propósito:** reordenar por altura entre clientes troca uma imagem
pior por um **403 na frente da congregação**, e essa decisão precisa de um
número, não de uma suposição — que é exatamente o que a nota `HAVIA` passa a
produzir. Fica em `ACHADOS-EM-ABERTO.md` §3.

**E a resposta operacional já existe:** numa rede fraca o conserto é **pedir
menos** no seletor **Online · 1080p · 720p · 480p** da folha, que vale para
"Tocar agora". Menos resolução são menos bytes por segundo — é assim que se
troca travamento por imagem menor num app que não faz isso sozinho.

### O que fecha o lote

Dois oráculos novos, os dois provados por reversão:

- `tools/telao-no-chao.test.mjs` — quatro estados (sem tela · telão no chão ·
  telão sobe · telão cai com a tela de pé). Sem a correção reprovam quatro
  asserções.
- `tools/espera-do-stream.test.mjs` — aparece · não pisca · só no stream · é
  contado. Sem o gate do primeiro `playing` a suíte trava no giro que nunca
  apaga; sem o atraso, três asserções reprovam.

48 → 50 oráculos verdes. `SHELL_VERSION` 59 — o campo `telao` é **forma de
retorno**, não método novo, e a regra vale igual. **EXIGE RELEASE v1.4.4.**
