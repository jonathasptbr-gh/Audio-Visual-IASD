...
**A RESOLUÇÃO NÃO FOI MEXIDA, e isso é decisão.** Ela é escolhida uma vez
(`YoutubeGrab.manifesto` pega a mais alta do PRIMEIRO cliente da ordem — visionOS
na frente, que é de onde vêm as URLs que o CDN serve) e o `mse.js` a serve até o
fim: não há ABR. Mexer em `ordemCliente` sem um aparelho na mão é apostar contra
a única coisa que faz o download funcionar. A resposta numa rede fraca **já
existe e é do operador**: o seletor **Online · 1080p · 720p · 480p** da folha
vale para "Tocar agora" (o `altura` viaja até o `ytStream`). E o Registro dá as
duas metades da conta para a próxima vez: `transmitindo Xp` (o que foi escolhido)
ao lado do `resumo` da extração com a maior altura POR CONTÊINER (o que havia).
