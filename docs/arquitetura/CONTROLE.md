...
- **A CIFRA é a ÚLTIMA da lista** (`lyricsViewSources`), e isso é a precedência
  ...
  **A VELOCIDADE ABRE UMA GAVETA, e não cicla** (v1.7.2). Pedido do operador:
...
- **A fração não custa parâmetro nenhum.** A linha já RECEBIA essa legenda — a
  ...
  ajuste.
- **E A LINHA NÃO DESENHA O NÚMERO** (v1.7.2). Ela teve DUAS formas de desenhá-lo
...
##### E O `⋮` CEDE A COLUNA AO TRABALHO (v1.7.2)
...
- **O símbolo da pergunta é a LIXEIRA** (`.row-slot--del`), e desde a v1.4.27 ela
  ...
  pelo mesmo caminho do ✓ do renomear. **A CAPA FICA À VISTA** (v1.7.2, pedido do
  ...
  tem duas.
- **E A LINHA SAI DE CENA quando o "sim" é tocado** (v1.7.2). Relato: *"ao tocar
...
#### O `⋮` cede a coluna ao processo (v1.4.27; a capa voltou na v1.7.2)
...
**A capa saiu na v1.4.27 e VOLTOU na v1.7.2, e as duas decisões estão certas** —
...
|---|---|---|---|
| exclusão | a LIXEIRA (`.row-slot--del`) | `<span>` inerte, `pointer-events: none` — ilustra, não decide; quem decide são os dois rótulos | **fica** (v1.7.2) |
...
dela, sem cobrir nada. Lá não há coluna a tomar emprestada, e **desde a v1.7.2 os
...
> **E A `object-URL` DE UMA MINIATURA É DO BLOB, NÃO DO RENDER** (v1.7.2).
