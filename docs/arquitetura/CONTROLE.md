...
| fonte muda | doze faixas SEM RESPOSTA (`semResposta`: o metadado que não veio OU a foto que não voltou) e nenhuma com: a passada PARA (`FUNDO_FONTE_MUDA`), e o Registro diz | sem teto, mil faixas com a fonte fora do ar seriam mil falhas a cada passada — e com o servidor de FOTOS mudo e o banco de pé, duas requisições por faixa |
