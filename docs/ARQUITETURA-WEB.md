...
| **Preview** (`.simple-stage`) | a projeção em miniatura, **só com tela conectada** — ver "A preview no lugar da seção de conexão". Com a busca fora da faixa (v5.200) ela ficaria com a largura inteira, então ganhou um teto de **20vh** de ALTURA: 16:9 num aparelho de 430px são ~242px, mais de um quarto da tela tirados da letra. O teto é de altura porque `--pv-ar` é a proporção do telão REAL (pode ser 2,17:1), e limitar a largura daria alturas diferentes conforme a TV |
| **Buscar música** (`#simpleSearchBtn`) | o MESMO popup de busca do acervo (`openHymnSearch`). Um toque na linha **toca a versão Cantada direto** (ver abaixo). **Desde a v5.200 ele mora ENTRE A LETRA E O TECLADO**, largo, nos dois estados: buscar é o começo de OPERAR, então pertence ao transporte — a milímetros do ▶ que vem logo depois de escolher — e não à faixa do alto, que é onde se resolve o que se resolve uma vez por culto |
